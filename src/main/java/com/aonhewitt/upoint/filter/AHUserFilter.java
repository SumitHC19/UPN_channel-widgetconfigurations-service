...
@WebFilter(urlPatterns = { "/channel/widgetConfigurations/*" })
public class AHUserFilter implements Filter {

	private static final Logger LOGGER = LoggerFactory.getLogger(AHUserFilter.class);
	...
	public void doFilter(ServletRequest request, ServletResponse response, FilterChain chain)
		...
		try {
			...
			/**
			 * Process alightSessionToken - Yogesh - TODO - verify Ahuser with
			 * for actual payload
			 */
			...
			String message = "*****" + httpRequest.getRequestURI() + " ---  Start Time =" + startTime
					+ " , Stop Time = " + endTime + " , Total Time = " + (endTime - startTime) + "mili seconds. MethodName:: doFilter()";
	...
	private void buildAHUserFromRequestToken(String alightSessionToken, String alightRequestHeader,
		...
		if (!ObjectUtils.isEmpty(alightSessionToken)) {
			try {
				...
				ErrorLogEventHelper.logErrorEvent(AHUserFilter.class.getName(), "Exception : "+ e.getMessage(), 
						"buildAHUserFromRequestToken()",
